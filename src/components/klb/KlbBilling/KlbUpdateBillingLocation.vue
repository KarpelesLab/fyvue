...
const submitEditBillingAddress = async () => {
  const _updateLocation = await rest<KlbUserLocationResult>(`User/Location/${billing.value?.User_Location__}`, "PATCH", {
  ...
  if (_updateLocation && _updateLocation.result == 'success') {
    const _updateBilling = await rest(`User/Billing/${billing.value?.User_Billing__}`, "PATCH", {
...
<template>
  <div v-if="isAuth">
    <form @submit.prevent="submitEditBillingAddress" class="klb-update-billing-loc" v-if="hasBilling">
      <div class="form-grid">
        <FyInput
          id="billingFirstname"
          :req="true"
