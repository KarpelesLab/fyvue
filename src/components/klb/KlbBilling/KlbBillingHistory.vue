...
const getPaymentHistory = async (page = 1) => {
  let _billingHistory = await rest<KlbBillingHistoryResult>("Order", "GET", {
    page_no: page,
    results_per_page: 10,
    Status: "completed",
  }).catch(() => { });
...
<template>
  <div class="klb-billing-history">
    ...
    <FyPaging id="billingHistory" :items="billingHistory.paging"
      v-if="billingHistory.paging && billingHistory.paging.page_no" class="billing-history-paging" />
    <FyTable :data="billingHistory.data" :headers="{
      ...
      <template v-slot:Actions_item="property">
        <a :href="property.data.item.Invoice_Url" target="_blank" class="btn neutral download-btn"
  ...
  <div v-if="billingHistory && billingHistory.data?.length == 0" class="no-billing-history">
    {{ $t('billing_history_empty') }}</div>
  </div>
</template>
