...
const useFVStore = defineStore({
    ...
    actions: {
        async refreshUser() {
            const apiData = await rest$1('User:get', 'GET').catch((err) => { });
    ...
    setup(__props) {
        ...
        const forgotPassword = async () => {
            if (await v$.value.$validate()) {
                const data = await rest$1('User:forgot_password', 'POST', {
        ...
        const userFlow = async (params = { initial: false }) => {
            ...
            response.value = await rest$1('User:flow', 'POST', formData.value).catch((err) => {
...
const rest = async (url, method = 'GET', params = {}, ctx = {}) => {
