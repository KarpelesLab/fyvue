...
const useHistory = defineStore({
  ...
  isSSRRendered() && initialState && initialState.piniaState && (pinia.state.value = initialState.piniaState), useHistory(pinia)._setRouter(router);
};
...
var _sfc_main$i = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    const props = __props, route = useRoute(), eventBus2 = useEventBus(), history = useHistory(), prevNextSeo = ref({}), url = getUrl(), fyhead = useFyHead(), isNewPage = (page2) => page2 >= 1 && page2 <= props.items.page_max && page2 != props.items.page_no, pageWatcher = ref(), next = () => {
    ...
    }), checkPageNumber(props.items.page_no), (_ctx, _cache) => __props.items && __props.items.page_max > 1 && __props.items.page_no ? (openBlock(), createElementBlock("div", _hoisted_1$h, [
    ...
    return (_ctx, _cache) => {
      ...
      />`),
          __props.single ? (openBlock(), createElementBlock("header", {
            key: 1,
            class: normalizeClass(["entry-header", (_a = __props.post.Top_Drive_Item) != null && _a.Media_Image ? "has-pic" : ""]),
...
const useSeo = (seo, initial = !1) => {
  initial && (seo.value.url = `${getUrl().scheme}://${getUrl().host}${getUrl().path}`, seo.value.canonical = `${getUrl().scheme}://${getUrl().host}${getUrl().path}`), seo.value.locale = getLocale$1().replace("-", "_"), seo.value.type || (seo.value.type = "website"), seo.value.robots = "index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1", useFyHead().lazySeo(seo.value, initial);
};
function useCMS() {
  return {
